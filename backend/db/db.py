...
<<<<<<< HEAD
context1 = Context(id=4, context_type="Online Asynchronous", building_blocks=[1, 2, 3], notations=[])
action1 = Action(id=5, action_type="Learner Action", source=1, target=2)
action2 = Action(id=6, action_type="Learner Action", source=2, target=3)
...
>>>>>>> 80138d321c59831adc03d626c9e7238411038cca
...
for lem in Lem.objects:
	...
	for context in lem.contexts:
		print("ID: %d, Context Type: %s, Building Blocks: %s, Notations: %s" % (context.id, context.context_type, context.building_blocks, 
			  context.notations))
	print("Actions: ")
	for action in lem.actions:
		print("ID: %d, Type: %s, Tail: %d, Head: %d" % (action.id, action.action_type, action.source, action.target))
	print("Notations: ")
